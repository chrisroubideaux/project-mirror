...
# ---------------------------------------------------
# Optional remote HF toxicity check
# Disabled by default for Aurora responsiveness
# ---------------------------------------------------

HF_TOXICITY_ENABLED = os.getenv("HF_TOXICITY_ENABLED", "false").lower() == "true"
...
ILLEGAL_PATTERNS = [
    ...
    r"\bmake a bomb\b",
    r"\bget away with murder\b",
    r"\bmake (meth|fentanyl)\b",
]

# ---------------------------------------------------
# Emotional distress (soft flag only)
# ---------------------------------------------------
...
def _hf_toxicity_score(text: str) -> Optional[float]:
    """
    Optional best-effort remote HF toxicity score.
    Disabled by default because it adds noticeable latency.
    Returns None on any failure.
    """
    if not HF_TOXICITY_ENABLED:
        return None

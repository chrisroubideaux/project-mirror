...
from services.aurora_speech import text_to_speech
from services.datetime_context import get_time_context   # NEW

aurora_bp = Blueprint("aurora", __name__, url_prefix="/api/aurora")


# ------------------------------------------------------------
# 1. GREETING (supports guest names + time-of-day)
# ------------------------------------------------------------
@aurora_bp.route("/greet", methods=["GET"])
def greet():
    ...
    tod = time_ctx["time_of_day"]      # morning / afternoon / evening / night
    day = time_ctx["day_name"]

    # Fallback if no name is provided
    name_word = guest_name if guest_name else "there"

    greeting = (
        f"Hi {name_word}. I’m Aurora. "
        f"It’s this {tod}, on {day}. "
...
# ------------------------------------------------------------
# 2. CONVERSATION (Whisper → GPT → ElevenLabs)
# ------------------------------------------------------------
@aurora_bp.route("/converse", methods=["POST"])
def converse():
    ...
    # 0) Optional guest name passed by frontend
    # e.g., formData.append("name", "Chris")
    ...
    # 2) Aurora brain (NOW includes guest_name)
    reply = aurora_whisper_reply(user_text, user_name=guest_name)
